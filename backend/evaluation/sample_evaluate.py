...
ResNet-50 modelini validation setinin bir sample'ı ile hızlıca değerlendirir.
"""
...
def create_sample_dataloader(data_dir, batch_size=32, sample_size=2000):
    """Create validation dataloader with random sampling"""
    ...
    if sample_size < total_size:
        ...
        print(f'📦 Sampled {sample_size} images from {total_size} total images')
    else:
        print(f'📦 Using all {total_size} images')
...
def evaluate_model(model, dataloader, device='cuda'):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            batch_count += 1
            if batch_count % 20 == 0:
                print(f'  Processed {batch_count} batches ({total} images)...')
...
def main():
    parser = argparse.ArgumentParser(description='Sample Evaluate ResNet-50 Model')
    ...
    print('\n' + '='*60)
    print('🚀 ResNet-50 Sample Evaluation')
    print('='*60)
    ...
    # Create dataloader
    dataloader, data_classes = create_sample_dataloader(args.data_dir, args.batch_size, args.sample_size)
    ...
    valid_classes = [(idx, acc) for idx, acc in sorted_classes if per_class_total[idx] >= 5]
    
    if len(valid_classes) > 0:
        print(f'\n✨ Best Performing Classes (Top-10):')
        for i, (class_idx, acc) in enumerate(valid_classes[:10], 1):
            print(f'  {i}. {model_classes[class_idx]:<30} {acc:>6.2f}% ({per_class_total[class_idx]} samples)')
    ...
    # Save results
    results_dir = 'runs/resnet50/evaluation'
    os.makedirs(results_dir, exist_ok=True)
    results_file = os.path.join(results_dir, 'sample_results.txt')
    
    with open(results_file, 'w', encoding='utf-8') as f:
        f.write('='*60 + '\n')
        f.write('ResNet-50 Sample Evaluation Results\n')
        f.write('='*60 + '\n\n')
        f.write(f'Sample Size: {len(preds)}\n')
        ...
        for class_idx, acc in sorted_classes:
            if per_class_total[class_idx] > 0:
                f.write(f'{model_classes[class_idx]:<30} {acc:>6.2f}% ({per_class_total[class_idx]} samples)\n')
